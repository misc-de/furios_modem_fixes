    ...
    async def doCreateBearer(self, properties):
        ...
        for ctx in contexts:
            ...
            if name.lower() == "internet":
                ...
                # Subscribe before touching Active, or the activation we are
                # about to ask for happens before anyone is listening.
                ofono_ctx_interface.on_property_changed(mm_bearer_interface.ofono_context_changed)
        ...
        if ofono_ctx is not None:
            ...
            # A context that is already up sends no signal, so read its state
            # once instead of waiting for a change that has already happened.
            # Settings first: Connected without an Interface is what makes
            # NetworkManager fail an activation with "missing data port".
            try:
                props = await ofono_ctx_interface.call_get_properties()
                for prop in ('Settings', 'IPv6.Settings', 'Active'):
                    if prop in props:
                        mm_bearer_interface.ofono_context_changed(prop, props[prop])
            except Exception as e:
                ofono2mm_print(f"Failed to read context {ofono_ctx}: {e}", self.verbose)
